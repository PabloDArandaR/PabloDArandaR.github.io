...
const setupData = (() => {
    ...
    // Initialize
    for (let node of cv) {
        nodes.set(node.id, { "location": { x: 0, y: 0 }, "level": -1, "parents": [], "nodeData": node })
    }
    ...
    // Add the location of each of the nodes
    for (let node of cv) {
...
const CVComponent = () => {
    ...
    useEffect(() => {
        ...
        let nodes, levels, links = setupData();

        var newNodes = new Map();
        let listTest = [1, 2, 3, 4, 5];
        setNodesData(new Map(nodes));
        setLevelsData(new Map(levels));
        setLinksData([...links])
        setLengthData(5);
    }, []);
    ...
    return (
        <div>
            <p className={styles.text}> Number of elements is: {nodesData.size}</p>
            <svg width="800" height="600" className={styles.nodeStyle}>
            </svg >
        </div >
